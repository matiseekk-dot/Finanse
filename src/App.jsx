...
const WeeklySummary = ({ transactions, month, cycleDay }) => {
  ...
  const [cycleStartStr, cycleEndStr] = getCycleRange(month, cycleDay);
  const cycleEndDate = new Date(cycleEndStr);
  const totalCycleDays = Math.max(1, Math.round((cycleEndDate - cycleStartDate) / 86400000) + 1);
  const daysInCycle = totalCycleDays;
  const daysLeft = Math.max(0, Math.round((cycleEndDate - todayDate) / 86400000));
  const dailyRate = daysElapsed > 0 ? cycleExp / daysElapsed : 0;
  const projectedExp = dailyRate * totalCycleDays;
