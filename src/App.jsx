...
const ForecastWidget = ({ income, expense, daysLeft, elapsedDays, transactions, month, cycleDay }) => {
  ...
  const burnRate  = elapsedDays > 0 ? expense / elapsedDays : 0;
  if (!isFinite(burnRate) || burnRate === 0) return null;

  // weekly burn rates for opt/pes
  const weekAgo = new Date(); weekAgo.setDate(weekAgo.getDate() - 7);
  const twoWeekAgo = new Date(); twoWeekAgo.setDate(twoWeekAgo.getDate() - 14);
  const toISO = d => d.toISOString().split("T")[0];
  const curTx = cycleTxs(transactions, month, cycleDay).filter(t => t.amount < 0 && t.cat !== "inne");
  const thisWkExp = curTx.filter(t => t.date >= toISO(weekAgo)).reduce((s,t)=>s+Math.abs(t.amount),0);
  const prevWkExp = curTx.filter(t => t.date >= toISO(twoWeekAgo) && t.date < toISO(weekAgo)).reduce((s,t)=>s+Math.abs(t.amount),0);

  const burnOpt = prevWkExp > 0 && thisWkExp < prevWkExp ? thisWkExp / 7 : burnRate * 0.8;
  const burnPes = thisWkExp > burnRate * 7 ? thisWkExp / 7 : burnRate * 1.25;

  const projBase = income - burnRate  * totalDays;
  const projOpt  = income - burnOpt   * totalDays;
  const projPes  = income - burnPes   * totalDays;
  ...
  return (
    <Card style={{ padding: "14px 16px" }}>
      ...
      <div style={{ marginTop: 8, fontSize: 11, color: "#475569", textAlign: "center" }}>
        Burn rate: {fmt(burnRate)}/dzien × {totalDays} dni
      </div>
    </Card>
  );
};
