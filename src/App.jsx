  ...
  return last6.map((ym) => {
    ...
    const txs =
      cycleDay <= 1
        ...
        : cycleTxs(transactions, mIdx, cycleDay).filter(
            (t) => t.cat !== "inne",
          );
    const income = txs
      .filter((t) => t.amount > 0)
...
const fmt = (n, showSign = false) => {
  const s = Math.abs(n).toLocaleString("pl-PL", {
  ...
  const handleExport = () => {
    ...
    const sumRows = months.map((m) => {
      const mTx = transactions.filter(
        (t) => t.date.startsWith(m) && t.cat !== "inne",
      );
      const income = mTx
        .filter((t) => t.amount > 0)
        .reduce((s, t) => s + t.amount, 0);
      ...
      return {
        Miesiąc: m,
        Przychody: +income.toFixed(2),
        Wydatki: +expense.toFixed(2),
        Bilans: +(income - expense).toFixed(2),
      };
    });
    ...
    if (goals && goals.length) {
      const goalRows = goals.map((g) => ({
        Nazwa: g.name,
        Cel_PLN: g.target,
        Odłożone_PLN: g.saved,
        Postęp:
          g.target > 0 ? `${((g.saved / g.target) * 100).toFixed(0)}%` : "0%",
        Emoji: g.emoji || "",
      }));
    ...
    const wsBackup = XLSX.utils.json_to_sheet([
      { JSON_backup: JSON.stringify(backupData) },
    ]);
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        setImportStatus("err");
        setImportMsg(
          "Błąd wczytywania pliku. Upewnij się, że to plik .xlsx z FinTrack.",
        );
      }
    };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Ustaw dzień miesiąca, od którego zaczyna się Twój cykl. Dzień{" "}
          <strong style={{ color: "#e2e8f0" }}>1</strong> = standardowy miesiąc
          kalendarzowy. Np. dzień{" "}
          <strong style={{ color: "#e2e8f0" }}>25</strong> → cykl "Kwiecień" to
          25 mar – 24 kwi.
        </p>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
          {[1, 10, 15, 20, 25, 27].map((d) => (
            <button
              ...
              style={{
                flex: 1,
                background: cycleDay === d ? "#1e3a5f" : "#060b14",
        ...
        >
          ...
          Transakcje, Konta, Budżety, Płatności, Podsumowanie, Cele + pełny
          backup JSON.
        </p>
        ...
        <button
          onClick={() => {
            ...
            const rows = Object.entries(cats)
              ...
              .map(
                ([cat, val]) =>
                  `<tr><td style="padding:4px 12px;border-bottom:1px solid #eee">${cat}</td><td style="padding:4px 12px;text-align:right;border-bottom:1px solid #eee">${val.toLocaleString("pl-PL", { minimumFractionDigits: 2 })} zł</td></tr>`,
              )
              .join("");
        ...
        >
          ...
          — zrób eksport przed importem jeśli chcesz zachować kopię.
        </p>
        ...
        >
          ...
          >
            🔔 Automatyczne pop-upy
          </div>
          <div style={{ fontSize: 12, color: "#64748b", lineHeight: 1.6 }}>
            Gdy otworzysz aplikację, automatycznie pojawi się żółty banner z
            listą płatności które są{" "}
            <strong style={{ color: "#fcd34d" }}>dziś</strong>,{" "}
            <strong style={{ color: "#fcd34d" }}>jutro</strong> lub{" "}
            <strong style={{ color: "#fcd34d" }}>za 3 dni</strong>. Działa bez
            żadnych uprawnień — tylko zamknij banner klikając ×.
          </div>
        </div>
        ...
        >
          Usuwa wszystkie Twoje dane i przywraca dane demonstracyjne z marca
          2026. Tej operacji nie można cofnąć.
        </p>
        <button
          onClick={() => {
            if (
              window.confirm("Na pewno? Wszystkie Twoje dane zostaną usunięte.")
            ) {
              if (
                window.confirm(
                  "Na pewno? Wszystkie dane zostaną usunięte — zapisz kopię przed resetem!",
                )
              ) {
            ...
            <div>
              <div
                style={{
                  fontWeight: 700,
                  fontSize: 13,
                  color: urgent ? "#fca5a5" : "#86efac",
                }}
              >
                {urgent
                  ? daysSinceLast === 1
                    ? "Nie dodałeś dziś transakcji!"
                    : `Brak transakcji od ${daysSinceLast} dni!`
...
const Dashboard = ({
  ...
  const histData = useMemo(
    () => buildHistData(transactions, cycleDay),
          ...
          >
            <span
              style={{
                ...
                fontSize: 30,
                fontWeight: 500,
                color: hideBalance ? "#1a2744" : "#e2e8f0",
      ...
      <Card>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <PieChart width={120} height={120}>
            <Pie
              data={accounts.map((a) => ({ name: a.name, value: a.balance }))}
              cx={55}
              cy={55}
              innerRadius={35}
              outerRadius={55}
...
const AccountsView = ({ accounts, setAccounts }) => {
  ...
  const saveAccount = () => {
    ...
    if (editAcc) {
      setAccounts((a) =>
        a.map((x) =>
          x.id === editAcc.id
            ? { ...x, ...form, balance: parseFloat(form.balance) }
            : x,
        ),
      );
    } else {
  ...
  const deleteAcc = (id) => {
    if (window.confirm("Usunąć to konto?"))
      ...
      <Card style={{ padding: "16px 18px" }}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            >
              {acc.type === "invest" ? (
                <TrendingUp size={17} color={acc.color} />
              ) : acc.type === "savings" ? (
                <PiggyBank size={17} color={acc.color} />
              ) : (
            ...
            <div>
              ...
              >
                {acc.bank} ·{" "}
                <Badge color={acc.color}>{typeLabel[acc.type]}</Badge>
              </div>
            </div>
          </div>
          <div style={{ textAlign: "right" }}>
  ...
  return (
    <div style={{ padding: "0 16px 100px" }}>
      {/* View switcher */}
      <div
        style={{ display: "flex", gap: 8, paddingTop: 8, paddingBottom: 14 }}
      >
        {[
          ["month", "📅 Bieżący"],
          ["period", "📊 Okresy"],
            ...
            style={{
              ...
              background:
                activeView === v
                  ...
                  : "#0f1825",
              border: `1px solid ${activeView === v ? "#2563eb" : "#1a2744"}`,
        ...
        <div>
            ...
            {Object.entries(PERIOD_LABELS).map(([v, l]) => (
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${period === v ? "#2563eb" : "#1a2744"}`,
          ...
          >
            {[
              ...
              { label: "Wpływy", val: periodIncTotal, color: "#10b981" },
              {
                label: "Bilans",
                val: periodIncTotal - periodTotal,
                color:
                  periodIncTotal - periodTotal >= 0 ? "#10b981" : "#ef4444",
              },
            ].map(({ label, val, color }) => (
              <div
                key={label}
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: `1px solid ${groupBy === v ? "#2563eb" : "#1a2744"}`,
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: `1px solid ${sortBy === v ? "#2563eb" : "#1a2744"}`,
          ...
          <Card>
            ...
            >
              ...
              <span
                style={{ color: "#334155", fontWeight: 400, marginLeft: 6 }}
              >
                ({groupedData.length})
              </span>
            </div>
                      ...
                      >
                        <div
                          style={{
                            ...
                            fontSize: 14,
                            fontWeight: 700,
                            color: groupBy === "cat" ? cat.color : "#e2e8f0",
                          }}
                        >
                          {fmt(row.total)}
                        </div>
      ...
      {activeView === "month" && (
        <div>
          ...
          <Modal
            ...
            title={editAcc ? "Edytuj konto" : "Nowe konto"}
          >
            <Input
              label="Nazwa konta"
              value={form.name}
...
const TransactionsView = ({
  ...
  const addTx = () => {
    ...
    const finalCat =
      form.type === "income"
        ? incomeCategories.includes(form.cat)
          ? form.cat
          : "przychód"
        : form.cat;
    const RATES = {
    ...
    if (
      form.type === "transfer" &&
      form.toAcc &&
      parseInt(form.toAcc) !== parseInt(form.acc)
    ) {
      ...
      const txOut = {
        id: Date.now(),
        date: form.date,
        desc: `Przelew → ${(accounts.find((a) => a.id === toId) || { name: toId }).name}`,
        amount: -rawAmt,
        cat: "inne",
        acc: fromId,
      };
      const txIn = {
        id: Date.now() + 1,
        date: form.date,
        desc: `Przelew ← ${(accounts.find((a) => a.id === fromId) || { name: fromId }).name}`,
        amount: rawAmt,
        cat: "inne",
        acc: toId,
      };
        ...
        setAccounts((accs) =>
          accs.map((a) => {
            if (a.id === fromId)
              return {
                ...a,
                balance: parseFloat((a.balance - rawAmt).toFixed(2)),
              };
            if (a.id === toId)
              return {
                ...a,
    ...
    if (editingId) {
          ...
          accs.map((a) => {
            ...
            if (a.id === oldTx.acc && a.id === txData.acc)
              return {
                ...a,
                balance: parseFloat(
                  (a.balance - oldTx.amount + txData.amount).toFixed(2),
                ),
              };
            if (a.id === oldTx.acc)
              return {
                ...a,
                balance: parseFloat((a.balance - oldTx.amount).toFixed(2)),
              };
            if (a.id === txData.acc)
              return {
                ...a,
                balance: parseFloat((a.balance + txData.amount).toFixed(2)),
              };
            return a;
          }),
        );
      ...
      setTransactions((tx) =>
        tx.map((t) => (t.id === editingId ? { ...t, ...txData } : t)),
      );
      setEditingId(null);
    } else {
    ...
    <div style={{ padding: "0 16px 100px" }}>
        ...
        >
          <div style={{ display: "flex", gap: 6 }}>
            {[
              ...
              <button
                ...
                onClick={() => setFilter(v)}
                style={{
                  background: filter === v ? "#1e3a5f" : "#0d1628",
                  border: `1px solid ${filter === v ? "#2563eb" : "#1a2744"}`,
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <button
              ...
              style={{
                background: showSearch ? "#1e3a5f" : "#0d1628",
                border: `1px solid ${showSearch ? "#2563eb" : "#1a2744"}`,
                color: showSearch ? "#60a5fa" : "#64748b",
                borderRadius: 8,
                padding: "6px 10px",
                cursor: "pointer",
              }}
          ...
          >
            ...
            >
              <button
                ...
                style={{
                  background: filterCat === "all" ? "#1e3a5f" : "#0d1628",
                  border: `1px solid ${filterCat === "all" ? "#2563eb" : "#1a2744"}`,
                  color: filterCat === "all" ? "#60a5fa" : "#64748b",
                  borderRadius: 8,
                  padding: "4px 10px",
                  cursor: "pointer",
                  fontSize: 11,
              ...
              {(allCats || CATEGORIES).map((c) => (
                <button
                  ...
                  style={{
                    background: filterCat === c.id ? c.color + "33" : "#0d1628",
                    border: `1px solid ${filterCat === c.id ? c.color : "#1a2744"}`,
                    color: filterCat === c.id ? c.color : "#64748b",
                    borderRadius: 8,
                    padding: "4px 10px",
                    cursor: "pointer",
                    fontSize: 11,
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      padding: "10px 0",
                      borderBottom:
                        i < txs.length - 1 ? "1px solid #0f1a2e" : "none",
                    }}
                  >
                    {/* Icon */}
                    <div
      ...
      >
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {[
            ["expense", "📤 Wydatek", "#ef4444"],
            ["income", "📥 Przychód", "#10b981"],
            ["transfer", "🔄 Przelew", "#60a5fa"],
          ].map(([v, l, c]) => (
            <button
              key={v}
              ...
              style={{
                flex: 1,
                background: form.type === v ? c + "22" : "#060b14",
                border: `1px solid ${form.type === v ? c : "#1a2744"}`,
                color: form.type === v ? c : "#64748b",
                borderRadius: 10,
                padding: 10,
                cursor: "pointer",
                fontWeight: 700,
        ...
        {form.type === "expense" && (
          ...
          >
            ...
            {(allCats || CATEGORIES)
              .filter((c) => c.group === "essential" && c.id !== "przychód")
              .map((c) => (
        ...
        <Select
          label={form.type === "transfer" ? "Z konta" : "Konto"}
          value={form.acc}
          onChange={(e) => setForm((f) => ({ ...f, acc: e.target.value }))}
        >
          {[...accounts]
            .sort((a, b) => {
        ...
        {form.type === "transfer" && (
          <Select
            label="Na konto"
            value={form.toAcc}
            onChange={(e) => setForm((f) => ({ ...f, toAcc: e.target.value }))}
          >
            {[...accounts]
              .filter((a) => a.id !== parseInt(form.acc))
              .sort((a, b) => {
                const order = { checking: 0, savings: 1, invest: 2 };
                return (order[a.type] || 1) - (order[b.type] || 1);
              })
...
const BudgetView = ({
  ...
  const addBudget = () => {
    ...
    setBudgets((b) => {
      ...
      if (exists >= 0) {
        const n = [...b];
        n[exists] = { ...n[exists], limit: parseFloat(form.limit) };
        return n;
      }
      return [
        ...b,
    ...
    <div style={{ padding: "0 16px 100px" }}>
      ...
      >
        <div>
          ...
          >
            {cycleDay > 1
              ? `Cykl ${fmtCycleLabel(month, cycleDay)}`
              : `Budżet · ${MONTH_NAMES[month]}`}
          </div>
          <div
            style={{
              fontFamily: "'DM Mono', monospace",
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        >
          ...
          <span
            style={{
              fontFamily: "'DM Mono', monospace",
              color: totalSpent / totalBudget > 0.9 ? "#ef4444" : "#10b981",
            }}
          >
            {totalBudget > 0
      ...
      >
        <Select
          ...
          onChange={(e) => setForm((f) => ({ ...f, cat: e.target.value }))}
        >
          {CATEGORIES.filter((c) => c.id !== "przychód" && c.id !== "inne").map(
            (c) => (
              <option key={c.id} value={c.id}>
                {c.label}
              </option>
...
const InvestmentsView = ({ accounts }) => {
  ...
  const chartData = XTB_PORTFOLIO.map((p) => ({
    name: p.ticker,
    value: p.valuePLN,
  }));
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              ...
              <span
                style={{
                  fontSize: 11,
                  background: isIKZE ? "#78350f22" : "#1e3a5f33",
                  color: isIKZE ? "#f59e0b" : "#60a5fa",
                  border: `1px solid ${isIKZE ? "#78350f66" : "#2563eb44"}`,
                  borderRadius: 5,
                  padding: "1px 6px",
                  fontWeight: 700,
                }}
          ...
          <div style={{ textAlign: "right", flexShrink: 0, marginLeft: 12 }}>
            ...
            <div
              style={{
                fontFamily: "'DM Mono', monospace",
                fontSize: 12,
                color: p.pnlPLN >= 0 ? "#10b981" : "#ef4444",
                marginTop: 3,
              }}
            >
              {p.pnlPLN >= 0 ? "+" : ""}
              {fmt(p.pnlPLN)} ({p.pnlPct >= 0 ? "+" : ""}
              {p.pnlPct.toFixed(2)}%)
            </div>
          </div>
        </div>
        ...
        {[
          {
            label: "Inwestycje",
            val: zwykleVal,
            pnl: zwykle.reduce((s, p) => s + p.pnlPLN, 0),
            color: "#8b5cf6",
          },
          {
            label: "IKZE",
            val: ikzeVal,
            pnl: ikze.reduce((s, p) => s + p.pnlPLN, 0),
            color: "#f59e0b",
          },
      ...
      <Card style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <PieChart width={110} height={110}>
            ...
            >
              {chartData.map((p) => (
                <Cell key={p.name} fill={COLORS[p.name] || "#64748b"} />
              ))}
            </Pie>
          </PieChart>
          <div style={{ flex: 1 }}>
            {XTB_PORTFOLIO.map((p) => (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <div
                    style={{
                      ...
                      height: 8,
                      borderRadius: 2,
                      background: COLORS[p.ticker] || "#64748b",
                    }}
                  />
                  <span
                    style={{
                ...
                <span style={{ fontSize: 11, color: "#64748b" }}>
                  {((p.valuePLN / totalValue) * 100).toFixed(0)}%
                </span>
              </div>
            ))}
          </div>
...
const RecurringReminder = ({
  ...
  const dueToday = payments.filter((p) => {
    ...
    if (p.freq === "monthly" || p.freq === "bimonthly")
      return (p.dueDay || p.dayOfMonth || 1) === dayOfMonth;
    return false;
  });

  // Check which ones already have a transaction today
  const notYetAdded = dueToday.filter(
    (p) =>
      !transactions.some(
        (t) =>
          t.desc === p.name && t.date === todayStr && t.amount === p.amount,
      ),
  );
...
const WeeklySummary = ({ transactions, month, cycleDay }) => {
  ...
  const daysElapsed = Math.max(
    1,
    Math.floor((todayDate - cycleStartDate) / 86400000) + 1,
  );
  const cycleTx = cycleTxs(transactions, month, cycleDay);
  const cycleExp = cycleTx
    .filter((t) => t.amount < 0 && t.cat !== "inne")
    .reduce((s, t) => s + Math.abs(t.amount), 0);
  const cycleInc = cycleTx
    .filter((t) => t.amount > 0 && t.cat !== "inne")
    .reduce((s, t) => s + t.amount, 0);
    ...
    <Card style={{ marginBottom: 14 }}>
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: 11, color: "#475569", marginTop: 3 }}>
            zostało {daysLeft} dni · {fmt(dailyRate)}/dzień
          </div>
        </div>
      </div>
      ...
      {topCat && (
        <div
          style={{
            background: getCat(topCat[0]).color + "15",
            border: `1px solid ${getCat(topCat[0]).color}33`,
            borderRadius: 10,
            padding: "8px 12px",
            fontSize: 12,
            color: "#94a3b8",
        ...
        >
          <span style={{ color: getCat(topCat[0]).color, fontWeight: 700 }}>
            {getCat(topCat[0]).label}
          </span>{" "}
          zjadło {((topCat[1] / thisWeek) * 100).toFixed(0)}% wydatków tygodnia
          ·{" "}
...
const ForecastTab = () => {
  ...
  const fvMthly =
    r > 0 ? monthly * ((Math.pow(1 + r, months) - 1) / r) : monthly * months;
      ...
      <Card style={{ marginBottom: 14 }}>
        ...
        <div>
          ...
          <input
            type="number"
            value={startAmt}
            onChange={(e) => setStartAmt(parseFloat(e.target.value) || 0)}
            style={{
              width: "100%",
              background: "#060b14",
              border: "1px solid #1a2744",
...
const GoalsView = ({
  ...
  const saveGoal = () => {
    ...
    const item = {
      ...
      accId: parseInt(form.accId),
      color: form.color,
      emoji: form.emoji,
    };
    if (editGoal) {
      setGoals((g) =>
        g.map((x) => (x.id === editGoal.id ? { ...x, ...item } : x)),
      );
    } else {
  ...
  const monthTx = cycleTxs(transactions, month, cycleDay).filter(
    (t) => t.amount < 0,
  );
  ...
  const addLimit = () => {
    ...
    if (existing) {
      setBudgets((b) =>
        b.map((x) =>
          x.cat === limitForm.cat
            ? { ...x, limit: parseFloat(limitForm.limit) }
            : x,
        ),
      );
    } else {
      setBudgets((b) => [
        ...
        {
          cat: limitForm.cat,
          limit: parseFloat(limitForm.limit),
          color: getCat(limitForm.cat).color,
        },
      ]);
    }
  ...
  const deleteLimit = (cat) =>
    setBudgets((b) => b.filter((x) => x.cat !== cat));

  return (
    <div style={{ padding: "0 16px 100px" }}>
      {/* Tab switcher */}
      <div
        style={{ display: "flex", gap: 6, paddingTop: 8, paddingBottom: 14 }}
      >
        {[
          ["goals", "🎯 Cele"],
          ["limits", "🚦 Limity"],
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      {[100, 500, 1000].map((amt) => (
                        <button
                          key={amt}
                          onClick={() => updateSaved(goal.id, amt)}
                          style={{
                            background: "#0a1e12",
          ...
          {budgets.length === 0 && (
            ...
            >
              ...
              <div style={{ fontSize: 12, color: "#334155", marginTop: 4 }}>
                Kliknij + Limit aby dodać pierwszy
              </div>
            </div>
          )}
      ...
      {activeTab === "vacation" &&
        (() => {
          ...
          const vacTx = transactions.filter(
            (t) =>
              t.amount < 0 &&
              ((t.date >= from &&
                t.date <= to &&
                vacation.categories.includes(t.cat)) ||
                pinned.includes(t.id)),
          );
          // Separate: in-range vs pre-trip pinned
          const inRange = vacTx.filter(
            (t) =>
              t.date >= from &&
              t.date <= to &&
              vacation.categories.includes(t.cat),
          );
          const preTrip = vacTx.filter(
            (t) =>
              pinned.includes(t.id) &&
              !(
                t.date >= from &&
          ...
          const preTripTotal = preTrip.reduce(
          ...
          const togglePin = (txId) => {
            ...
            saveVacation({
              ...vacation,
              pinnedTxIds: curr.includes(txId)
                ? curr.filter((x) => x !== txId)
                : [...curr, txId],
            });
          };
          ...
          const totalDays =
            dFrom && dTo ? Math.max(1, Math.ceil((dTo - dFrom) / 86400000)) : 0;
          const daysPassed = dFrom
            ? Math.max(0, Math.ceil((today - dFrom) / 86400000))
            : 0;
          const daysLeft = dTo
            ? Math.max(0, Math.ceil((dTo - today) / 86400000))
            : 0;
          const dailyBudget =
            budget > 0 && totalDays > 0 ? budget / totalDays : 0;
          const dailyLeft =
            remaining > 0 && daysLeft > 0 ? remaining / daysLeft : 0;
          const isActive = dFrom && dTo && today >= dFrom && today <= dTo;

          const ALL_CATS = [...CATEGORIES];
          const vacCats = ALL_CATS.filter(
            (c) =>
              ![
                "przychód",
                "sprzedaż",
            ...
            <div>
              ...
              <Card style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <input
                      value={vacation.name}
                      onChange={(e) =>
                        saveVacation({ ...vacation, name: e.target.value })
                      }
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <input
                      value={vacation.dest}
                      onChange={(e) =>
                        saveVacation({ ...vacation, dest: e.target.value })
                      }
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        saveVacation({ ...vacation, dateFrom: e.target.value })
                      }
                      style={{
                        width: "100%",
                        background: "#060b14",
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        saveVacation({ ...vacation, dateTo: e.target.value })
                      }
                      style={{
                        width: "100%",
                        background: "#060b14",
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <input
                    type="number"
                    value={vacation.budget}
                    onChange={(e) =>
                      saveVacation({ ...vacation, budget: e.target.value })
                    }
                    placeholder="np. 5000"
                    style={{
                      width: "100%",
              ...
              {budget > 0 && (
                <Card
                  style={{
                    marginBottom: 14,
                    background: isActive
                      ? "linear-gradient(135deg,#0a1e12,#061610)"
                      : undefined,
                  }}
                >
                  {isActive && (
                  ...
                  >
                    {[
                      ...
                      },
                      {
                        label: "Dziś mogę",
                        val: dailyLeft,
                        color: dailyLeft < 0 ? "#ef4444" : "#f59e0b",
                        suffix: "/dzień",
                      },
              ...
              <Card style={{ marginBottom: 14 }}>
                ...
                >
                  ✈️ Przed wyjazdem{" "}
                  {preTripTotal > 0 && `· ${fmt(preTripTotal)}`}
                </div>
                <div
                  style={{ fontSize: 11, color: "#475569", marginBottom: 10 }}
                >
                  Zaznacz transakcje które należą do wyjazdu (loty,
                  ubezpieczenie, hotel...)
                </div>
                {/* Date range filter */}
                <div
                    ...
                    <div style={{ display: "flex", gap: 4 }}>
                      {[
                        ...
                        return (
                          <button
                            key={l}
                            onClick={() => setCandidateFrom(df)}
                            style={{
                              background:
                                candidateFrom === df ? "#1e3a5f" : "#060b14",
                              border: `1px solid ${candidateFrom === df ? "#2563eb" : "#1a2744"}`,
                              borderRadius: 6,
                              padding: "5px 8px",
                              cursor: "pointer",
                              color:
                                candidateFrom === df ? "#60a5fa" : "#475569",
                              fontSize: 11,
                              fontWeight: 700,
                            }}
                          >
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                <button
                  onClick={() => {
                    if (!vacation.name) {
                      alert("Wpisz nazwę wyjazdu przed zapisem");
                      return;
                    }
                    const entry = {
                      ...vacation,
                ...
                <button
                  onClick={() => {
                    if (
                      !window.confirm("Wyczyścić obecny wyjazd i zacząć nowy?")
                    )
                      return;
                    const empty = {
                      name: "",
                ...
                <div style={{ marginTop: 16 }}>
                  ...
                  >
                    {vacationArchive.map((v) => {
                      ...
                      const pct =
                        bgt > 0 ? Math.min(100, (v.spent / bgt) * 100) : 0;
                      ...
                      const totalDays =
                        v.dateFrom && v.dateTo
                          ? Math.max(
                              1,
                              Math.ceil(
                                (new Date(v.dateTo) - new Date(v.dateFrom)) /
                                  86400000,
                              ),
                            )
                          : 0;
                      ...
                      const vTxs = transactions
                        .filter(
                          (t) =>
                            t.amount < 0 &&
                            ((t.date >= vFrom &&
                              t.date <= vTo &&
                              vCats.includes(t.cat)) ||
                              vPins.includes(t.id)),
                        )
                        .sort((a, b) => b.date.localeCompare(a.date));
                      ...
                      vTxs.forEach((t) => {
                        catBreakdown[t.cat] =
                          (catBreakdown[t.cat] || 0) + Math.abs(t.amount);
                      });
                      const catRows = Object.entries(catBreakdown).sort(
                        (a, b) => b[1] - a[1],
                      );
                      ...
                      return (
                        <div
                          ...
                          style={{
                            background: "#0f1825",
                            border: `1px solid ${isOpen ? "#2563eb44" : "#1a2744"}`,
                            borderRadius: 14,
                            overflow: "hidden",
                          }}
                        >
                          ...
                          <div
                            onClick={() =>
                              setExpandedVacId(isOpen ? null : v.id)
                            }
                          ...
                          >
                            ...
                            >
                              ...
                              <button
                                onClick={(e) => {
                                  ...
                                  if (
                                    window.confirm(
                                      `Usunąć "${v.name}" z archiwum?`,
                                    )
                                  )
                                    setVacationArchive((a) =>
                                      a.filter((x) => x.id !== v.id),
                                    );
                                }}
                                style={{
                                  background: "none",
                            ...
                            >
                              ...
                              <span
                                style={{
                                  color: pct > 100 ? "#ef4444" : "#10b981",
                                }}
                              >
                                Wydano: <strong>{fmt(v.spent)}</strong> (
                                {pct.toFixed(0)}%)
                              </span>
                            </div>
                            {bgt > 0 && (
      ...
      >
            ...
            {["🏖️", "💻", "🚗", "🏠", "💍", "✈️", "📈", "🎯", "🎸", "👶"].map(
              (e) => (
                <button
                  ...
                  style={{
                    fontSize: 22,
                    background: form.emoji === e ? "#1e3a5f" : "#060b14",
                    border: `1px solid ${form.emoji === e ? "#2563eb" : "#1a2744"}`,
                    borderRadius: 8,
                    padding: "4px 8px",
                    cursor: "pointer",
                  }}
        ...
        <Select
          label="Konto"
          value={form.accId}
          onChange={(e) => setForm((f) => ({ ...f, accId: e.target.value }))}
        >
          {accounts.map((a) => (
            <option key={a.id} value={a.id}>
              {a.name}
      ...
      >
        ...
        >
          {CATEGORIES.filter(
            (c) =>
              !["przychód", "inne", "sprzedaż", "kiga", "bukmacherka"].includes(
                c.id,
              ),
          ).map((c) => (
            <option key={c.id} value={c.id}>
        ...
        <Input
          ...
          onChange={(e) =>
            setLimitForm((f) => ({ ...f, limit: e.target.value }))
          }
          placeholder="np. 200"
        />
        <div
...
const PaymentsView = ({
  ...
  const save = () => {
    ...
    if (editItem)
      setPayments((p) => p.map((x) => (x.id === editItem.id ? item : x)));
  ...
  const freqLabel = (item) => {
    ...
    if (item.freq === "weekly")
      return `Co tydzień · ${DAYS_PL[(item.dayOfWeek - 1) % 7]}`;
    if (item.freq === "bimonthly") {
      const startM = item.startMonth || 0;
      let next = -1;
      for (let i = 1; i <= 12; i++) {
        const m = (month + i) % 12;
        if (Math.abs(m - startM) % 2 === 0) {
          next = m;
          break;
        }
      }
  ...
  const ItemCard = ({ item }) => {
    ...
    const overdue =
      !p && isMonthly && isCurrentMonth && (item.dueDay || 1) < TODAY_DAY;
    const soon =
      !p &&
      !overdue &&
      isMonthly &&
      isCurrentMonth &&
      (item.dueDay || 1) - TODAY_DAY <= 3 &&
      (item.dueDay || 1) >= TODAY_DAY;
    return (
      <div
        style={{
          background: p
      ...
      >
        ...
        >
          {p ? (
            ...
            <Circle size={9} color={overdue ? "#ef444488" : "#334155"} />
          )}
        </button>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div
            style={{
              fontSize: 11,
              marginTop: 2,
              color: overdue ? "#ef4444" : soon ? "#f59e0b" : "#475569",
              display: "flex",
              alignItems: "center",
              gap: 5,
              flexWrap: "wrap",
          ...
          >
            <span>
              {overdue
                ? "! Termin minal (" + item.dueDay + ".)"
                : soon
                  ? `🕐 Za ${item.dueDay - TODAY_DAY} dni`
                  : freqLabel(item)}
            </span>
            {item.freq === "bimonthly" && (
              <span
  ...
  return (
    <div style={{ padding: "0 16px 100px" }}>
      ...
      <div style={{ paddingTop: 8, paddingBottom: 14 }}>
        ...
        >
          <button
            onClick={() => setLocalMonth((m) => Math.max(0, m - 1))}
            style={{
              background: "#1a2744",
              border: "none",
              borderRadius: 8,
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <div
              style={{
                fontSize: 16,
                fontWeight: 700,
                color: isCurrentMonth ? "#e2e8f0" : "#60a5fa",
                marginTop: 2,
              }}
            >
              {MONTH_NAMES[month]} 2026{" "}
              {isCurrentMonth && (
                <span style={{ fontSize: 10, color: "#10b981" }}>● teraz</span>
              )}
            </div>
          </div>
          <button
            onClick={() => setLocalMonth((m) => Math.min(11, m + 1))}
            style={{
              background: "#1a2744",
              border: "none",
              borderRadius: 8,
        ...
        >
          {showAll
            ? "👁 Wszystkie płatności (edycja/usuwanie)"
            : "Pokaż tylko aktywne w tym miesiącu"}
        </button>
        <div
          style={{
            fontFamily: "'DM Mono', monospace",
      ...
      {(() => {
        const inactive = payments.filter(
          (p) => p.freq === "bimonthly" && !isActiveThisMonth(p),
        );
      ...
      >
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
            {[
              ...
              <button
                ...
                style={{
                  flex: 1,
                  background: form.type === t ? "#1e3a5f" : "#060b14",
        ...
        <Select
          ...
          onChange={(e) => setForm((f) => ({ ...f, cat: e.target.value }))}
        >
          {CATEGORIES.filter((c) => !["przychód", "inne"].includes(c.id)).map(
            (c) => (
              <option key={c.id} value={c.id}>
                {c.label}
              </option>
        ...
        {form.freq === "bimonthly" && (
          <Select
            label="Pierwszy miesiąc płatności"
            value={form.startMonth || new Date().getMonth()}
            onChange={(e) =>
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 11, color: "#475569", marginTop: 2 }}>
              Checkbox do odznaczania co miesiąc
            </div>
          </div>
          <button
            onClick={() => setForm((f) => ({ ...f, trackPaid: !f.trackPaid }))}
            style={{
              width: 44,
              height: 24,
              borderRadius: 12,
              border: "none",
              cursor: "pointer",
              background: form.trackPaid ? "#10b981" : "#1a2744",
              position: "relative",
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 11, color: "#475569", marginTop: 2 }}>
              Liczy się do rozliczenia wspólnego
            </div>
          </div>
          <button
            ...
            style={{
              ...
              border: "none",
              cursor: "pointer",
              background: form.shared ? "#10b981" : "#1a2744",
              position: "relative",
              transition: "background 0.2s",
            }}
          >
...
const MonthComparison = ({ transactions, month }) => {
  ...
  const cmpTx = transactions.filter(
    (t) => t.date.startsWith(cmpKey) && t.cat !== "inne",
  );
  const curTx = transactions.filter(
    (t) => t.date.startsWith(curKey) && t.cat !== "inne",
  );
  ...
  return (
    <Card style={{ marginBottom: 14 }}>
      ...
      >
        ...
        <select
          value={cmpMonth}
          onChange={(e) => setCmpMonth(parseInt(e.target.value))}
          style={{
            flex: 1,
            background: "#060b14",
            border: "1px solid #1a2744",
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: "#475569" }}>
                      {MONTHS[cmpMonth]}: {fmt(row.cmp)} → {MONTHS[month]}:{" "}
                      {fmt(row.cur)}
                    </div>
                  </div>
                  <div
                    style={{
                      ...
                      fontSize: 12,
                      fontWeight: 700,
                      color: row.diff > 0 ? "#ef4444" : "#10b981",
                      flexShrink: 0,
                    }}
                  >
                    {row.diff > 0 ? "+" : ""}
...
const AnalyticsView = ({
  ...
  const getPeriodTx = () => {
    ...
    return transactions.filter(
      (t) => t.date >= from && t.date <= to && t.cat !== "inne",
    );
  };
  ...
  const catData = useMemo(() => {
    ...
    return Object.entries(map)
      .map(([cat, val]) => ({ cat, val, ...getCat(cat) }))
      .sort((a, b) => b.val - a.val);
  }, [expense]);
  ...
  return (
    <div style={{ padding: "0 16px 100px" }}>
      <div style={{ paddingTop: 8, paddingBottom: 16 }}>
        ...
        >
          Analityka ·{" "}
          {cycleDay > 1 ? fmtCycleLabel(month, cycleDay) : MONTH_NAMES[month]}
        </div>
      </div>
      ...
      >
        {[
          ...
          {
            label: "Bilans",
            val: fmt(totalInc - totalExp, true),
            color: totalInc >= totalExp ? "#10b981" : "#ef4444",
          },
          {
            label: "Stopa oszcz.",
        ...
        <ResponsiveContainer width="100%" height={110}>
          <BarChart data={dayData}>
            ...
            <Tooltip
              ...
              formatter={(v) => [fmt(v), "Wydatki"]}
            />
            <Bar dataKey="v" fill="#3b82f644" radius={[3, 3, 0, 0]}>
              {dayData.map((_, i) => (
                <Cell key={i} fill={(_) => "#3b82f6"} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
          ...
          return (
            <div
              ...
              style={{
                ...
                padding: "8px 0",
                borderBottom:
                  i < catData.length - 1 ? "1px solid #0f1a2e" : "none",
              }}
            >
              <div
                style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                <div
                  style={{ background: "#060b14", borderRadius: 3, height: 4 }}
                >
                  <div
                    style={{
                      width: `${pct}%`,
      ...
      {(() => {
        ...
        transactions
          ...
          .forEach((t) => {
            ...
            if (!shopMap[key])
              shopMap[key] = { count: 0, total: 0, cat: t.cat };
            shopMap[key].count++;
      ...
      {(() => {
        ...
        if (sharedItems.length === 0)
          return (
            <Card style={{ marginTop: 0 }}>
              ...
              >
                Brak wspólnych rachunków — zaznacz „Wspólne z Kingą" w
                Płatnościach
              </div>
            </Card>
          );

        const totalShared = sharedItems.reduce(
        ...
        return (
          <Card style={{ marginTop: 0 }}>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 10,
                      border: `1px solid ${isPd ? "#16a34a33" : "#1a2744"}`,
                    }}
                  >
                    <div>
                      <span
                        style={{
                          fontSize: 13,
                          color: isPd ? "#475569" : "#e2e8f0",
                          textDecoration: isPd ? "line-through" : "none",
                        }}
                      >
                        {item.name}
                      </span>
            ...
            <div
              style={{ height: 1, background: "#1a2744", marginBottom: 14 }}
            />
...
export default function App() {
      ...
      .then((d) => {
        try {
          if (d) {
            ...
            if (Array.isArray(d.budgets) && d.budgets.length)
            ...
            if (d.month != null && d.month >= 0 && d.month <= 11)
              setMonth(d.month);
            if (d.cycleDay != null && d.cycleDay >= 1 && d.cycleDay <= 28)
              setCycleDay(d.cycleDay);
          }
        } catch (e) {
  ...
  const handleLoadFile = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      if (!d) {
        setImportErr("Nieprawidłowy plik — wybierz plik .json z FinTrack");
        return;
      }
      ...
      <div style={{ paddingBottom: 20 }}>
        ...
        {tab === "transactions" && (
          <TransactionsView
            ...
            _forceOpenModal={fabOpen}
            _onModalClose={() => setFabOpen(false)}
          />
        )}
        {tab === "payments" && (
          <PaymentsView
        ...
        {tab === "goals" && (
          <GoalsView
            ...
            setVacationArchive={setVacationArchive}
          />
        )}
        {tab === "analytics" && (
          <AnalyticsView
