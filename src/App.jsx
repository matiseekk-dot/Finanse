...
const TransactionsView = ({ transactions, setTransactions, accounts, setAccounts, _forceOpenModal, _onClose, _onModalClose }) => {
  ...
  const [form, setForm] = useState({ date: new Date().toISOString().split("T")[0], desc: "", amount: "", cat: "jedzenie", acc: 1, type: "expense", tags: "" });
  ...
  const addTx = () => {
    ...
    const txData = { date: form.date, desc: form.desc, amount: parseFloat(amt.toFixed(2)), cat: finalCat, acc: parseInt(form.acc), tags };
    if (editingId) {
      ...
      <div style={{ display: "flex", gap: 8, paddingTop: 8, paddingBottom: 14 }}>
        {[["goals","🎯 Cele"],["limits","🚦 Limity"]].map(([t,l]) => (
          <button key={t} onClick={() => setActiveTab(t)} style={{
            flex: 1, padding: "10px 0", borderRadius: 12, cursor: "pointer", fontWeight: 700, fontSize: 13,
            fontFamily: "'Space Grotesk', sans-serif",
