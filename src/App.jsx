...
function schedulePaymentNotifications(payments, paid, month) {
  ...
  // Fire only once per day — store last fire date in localStorage
  const today       = new Date();
  const todayStr    = today.toISOString().slice(0, 10);
  const lastFired   = localStorage.getItem("ft_notif_date");
  ...
  if (fired) localStorage.setItem("ft_notif_date", todayStr);
}
