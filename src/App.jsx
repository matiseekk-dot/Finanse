...
const INITIAL_TEMPLATES = [
  { id: 1, desc: "Żabka",        amount: 30,    cat: "zakupy",    acc: 1 },
  { id: 2, desc: "Paliwo",       amount: 200,   cat: "transport", acc: 1 },
  { id: 3, desc: "Restauracja",  amount: 80,    cat: "jedzenie",  acc: 1 },
  { id: 4, desc: "Kupon",        amount: 20,    cat: "bukmacher", acc: 1 },
  { id: 5, desc: "Kawa",         amount: 15,    cat: "kawiarnia", acc: 1 },
];
      ...
      <div style={{ background: "#0d1628", border: "1px solid #1a2744", borderRadius: "20px 20px 0 0",
        ...
        {(() => {
          const [tpls, setTpls] = React.useState(() =>
            JSON.parse(localStorage.getItem("ft_templates") || JSON.stringify(INITIAL_TEMPLATES))
          );
          const [tForm, setTForm] = React.useState({ desc: "", amount: "", cat: "zakupy" });
          const saveTpls = (next) => {
            setTpls(next);
            localStorage.setItem("ft_templates", JSON.stringify(next));
          };
          return (
            <div>
              {/* List */}
                  ...
                  return (
                    <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 10,
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: "#475569" }}>{cat.label} · {t.amount} zł</div>
                      </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, background: "#060b14",
                ...
                <Input label="Nazwa (np. Żabka)" value={tForm.desc}
                  onChange={e => setTForm(f => ({...f, desc: e.target.value}))} placeholder="np. Żabka"/>
                <Input label="Kwota (zł)" type="number" value={tForm.amount}
...
const Dashboard = ({ accounts, transactions, setTransactions, payments, month, setMonth, onAddTx, cycleDay = 1 }) => {
  ...
  const dailySpend  = elapsedCycDays > 0 ? expense / elapsedCycDays : 0;
  ...
  return (
    <div style={{ padding: "0 16px 100px", display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <Card style={{ padding: "16px 18px" }}>
        ...
        {/* Big "pozostało" tile */}
        <div style={{
          background: balance >= 0 ? "linear-gradient(135deg,#0a1e12,#052e16)" : "linear-gradient(135deg,#1a0808,#200e0e)",
          border: `1px solid ${balance >= 0 ? "#16a34a44" : "#7f1d1d44"}`,
          borderRadius: 14, padding: "14px 16px", marginBottom: 10,
          display: "flex", justifyContent: "space-between", alignItems: "center",
        }}>
          <div>
            <div style={{ fontSize: 10, color: "#475569", fontWeight: 700, textTransform: "uppercase", marginBottom: 4 }}>
              {balance >= 0 ? "Pozostalo w tym cyklu" : "Przekroczono budzet"}
            </div>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 26, fontWeight: 700,
              color: balance >= 0 ? "#10b981" : "#ef4444" }}>
              {fmt(Math.abs(balance))}
            </div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 10, color: "#475569", marginBottom: 4 }}>Wydano</div>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 15, fontWeight: 700,
              color: spendPct > 100 ? "#ef4444" : spendPct > 80 ? "#f59e0b" : "#94a3b8" }}>
              {spendPct.toFixed(0)}%
            </div>
            <div style={{ fontSize: 10, color: "#334155" }}>przychodów</div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
          <div style={{ background: "#060b14", borderRadius: 10, padding: "10px 12px" }}>
            <div style={{ fontSize: 9, color: "#475569", fontWeight: 700, textTransform: "uppercase", marginBottom: 4 }}>Wydajesz dziennie</div>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 15, fontWeight: 700,
              color: dailySpend <= dailyBudget ? "#10b981" : dailySpend <= dailyBudget * 1.2 ? "#f59e0b" : "#ef4444" }}>
              {fmt(dailySpend)}
            </div>
          </div>
          <div style={{ background: "#060b14", borderRadius: 10, padding: "10px 12px" }}>
    ...
    <div style={{ padding: "0 16px 100px" }}>
      ...
      <div style={{ overflowX: "auto", whiteSpace: "nowrap", paddingBottom: 8, paddingTop: 8,
        ...
        <div style={{ display: "inline-flex", gap: 6, paddingLeft: 0 }}>
          {((() => { try { return JSON.parse(localStorage.getItem("ft_templates") || JSON.stringify(INITIAL_TEMPLATES)); } catch(_) { return INITIAL_TEMPLATES; } })()).map(tpl => (
...
const PaymentsView = ({ payments, setPayments, paid, setPaid, transactions, setTransactions, accounts, month: globalMonth }) => {
  ...
  const togglePaid = (item) => {
    ...
    if (nowPaid) {
      ...
      const date     = isCurrentMonth ? todayStr : dueStr;
...
const AnalyticsView = ({ transactions, payments, paid, month, cycleDay = 1 }) => {
  ...
  return (
    <div style={{ padding: "0 16px 100px" }}>
      ...
      {(() => {
        const shopMap = {};
        const skipCats = ["inne", "inwestycje", "przychód", "sprzedaż", "kiga", "bukmacherka"];
        transactions
      ...
      {/* Por wnanie miesi cy */}
      {/* Trend kategorii - 6 cykli */}
      {(() => {
        const [trendCat, setTrendCat] = React.useState("jedzenie");
              ...
              {cats6.map(({ m, val }, i) => {
                ...
                return (
                  <div key={m} style={{ flex: 1, display: "flex", flexDirection: "column",
                    ...
                    <div style={{ width: "100%", background: "#060b14", borderRadius: "4px 4px 0 0",
