...
const SettingsPanel = ({ open, onClose, accounts, transactions, budgets, payments, paid,
  ...
  const handleExport = () => {
    ...
    // Sheet 7: Pełny backup JSON (wszystkie dane)
    const backupData = {
      ...
      <Modal open={modal} onClose={() => { setModal(false); setEditingId(null); }} title={editingId ? "Edytuj transakcję" : "Nowa transakcja"}>
        ...
        {form.type === "expense" && (
          <Select label="Kategoria" value={form.cat} onChange={e => setForm(f => ({...f, cat: e.target.value}))}>
            {(allCats||CATEGORIES).filter(c => c.id !== "przychód").map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
          </Select>
        )}
...
const PaymentsView = ({ payments, setPayments, paid, setPaid, transactions, setTransactions, accounts, month: globalMonth }) => {
  ...
  const save = () => {
    ...
    const item = {
      ...
      startMonth: form.freq === "bimonthly" ? (form.startMonth ?? new Date().getMonth()) : 0,
    };
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, marginTop: 2, color:
            ...
            <span>{overdue ? `⚠ Termin minął (${item.dueDay}.)` :
  ...
  return (
    <div style={{ padding: "0 16px 100px" }}>
      ...
      {/* Nieaktywne w tym miesiącu — co 2 miesiące */}
      {(() => {
...
// ── NOTIFICATIONS ────────────────────────────────────────────────────────────
// Get payments due soon (0=today, 1=tomorrow, 3=in 3 days)
function getUpcomingPayments(payments, paid, month) {
  const today    = new Date();
  const todayDay = today.getDate();
  const monthKey = `2026-${String(month+1).padStart(2,"0")}`;
  const due = [];
  payments
    .filter(p => p.trackPaid && (p.freq === "monthly" || p.freq === "bimonthly"))
    .filter(p => !paid[`${p.id}_${monthKey}`])
    .forEach(p => {
      const daysUntil = (p.dueDay || 1) - todayDay;
      if (daysUntil >= 0 && daysUntil <= 3) {
        due.push({ ...p, daysUntil });
      }
    });
...
export default function App() {
  ...
  useEffect(() => {
    if (!loaded) return;
    const due = getUpcomingPayments(payments, paid, month);
    setDueReminders(due);
  }, [loaded, payments, paid, month]);
  ...
  return (
    <div style={{ fontFamily: "'Space Grotesk', sans-serif", background: "#060b14", color: "#e2e8f0", minHeight: "100vh", maxWidth: 480, margin: "0 auto", position: "relative", overflowX: "hidden" }}>
      ...
      {/* Settings panel */}
      {/* Payment due reminder popup */}
      {dueReminders.filter(r => !dismissedReminders.includes(r.id)).length > 0 && (
        <div style={{ position: "fixed", top: 60, left: 12, right: 12, zIndex: 300,
          background: "linear-gradient(135deg,#1a1208,#221a08)",
          border: "1px solid #78350f",
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <button onClick={() => setDismissedReminders(dueReminders.map(r => r.id))}
              style={{ background: "none", border: "none", cursor: "pointer", color: "#92400e", fontSize: 18 }}>×</button>
          </div>
          {dueReminders.filter(r => !dismissedReminders.includes(r.id)).map(r => (
            <div key={r.id} style={{ display: "flex", justifyContent: "space-between",
              alignItems: "center", padding: "7px 0",
              borderBottom: "1px solid #78350f33" }}>
              <div>
                <span style={{ fontSize: 13, color: "#fde68a", fontWeight: 600 }}>{r.name}</span>
                <span style={{ fontSize: 11, color: "#92400e", marginLeft: 8 }}>
                  {r.daysUntil === 0 ? "⚠ dziś!" : r.daysUntil === 1 ? "jutro" : `za ${r.daysUntil} dni`}
                </span>
              </div>
              <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 13, color: "#f59e0b", fontWeight: 700 }}>
                {fmt(Math.abs(r.amount))}
              </span>
            </div>
          ))}
          <button onClick={() => { setTab("payments"); setDismissedReminders(dueReminders.map(r => r.id)); }}
            style={{ marginTop: 10, width: "100%", background: "#f59e0b", border: "none",
