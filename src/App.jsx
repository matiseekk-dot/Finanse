...
const TransactionsView = ({ transactions, setTransactions, accounts, setAccounts, allCats, _forceOpenModal, _onClose, _onModalClose }) => {
  ...
  const [form, setForm] = useState({ date: new Date().toISOString().split("T")[0], desc: "", amount: "", cat: "jedzenie", acc: 1, type: "expense", currency: "PLN" });
...
const GoalsView = ({ goals, setGoals, accounts, budgets, setBudgets, transactions, month, vacationArchive = [], setVacationArchive }) => {
  const [modal,       setModal]       = useState(false);
  ...
  const [candidateTo, setCandidateTo] = useState(() => {
    const stored = JSON.parse(localStorage.getItem("ft_vacation") || "null");
    return stored?.dateFrom || new Date().toISOString().slice(0,10);
  });
  ...
  return (
    <div style={{ fontFamily: "'Space Grotesk', sans-serif", background: "#060b14", color: "#e2e8f0", minHeight: "100vh", maxWidth: 480, margin: "0 auto", position: "relative", overflowX: "hidden" }}>
      ...
      <div style={{ paddingBottom: 20 }}>
        ...
        {tab === "goals"        && <GoalsView goals={goals} setGoals={setGoals} accounts={accounts} budgets={budgets} setBudgets={setBudgets} transactions={transactions} month={month} vacationArchive={vacationArchive} setVacationArchive={setVacationArchive}/>}
